...
def get_operation(op_op2, op_num2, itself):
    if op_op2 == '*':
        if itself:
            operation2 = lambda old: prime_factors(get_number(old) + get_number(old))
        else:
            operation2 = lambda old: old + prime_factors(op_num2)
    elif op_op2 == '+':
        if itself:
            operation2 = lambda old: old.append(2)
        else:
            operation2 = lambda old: prime_factors(get_number(old) + op_num2)
    elif op_op2 == '-':
        if itself:
...
def get_throw_to(test_num2, true_test_monkey2, false_test_monkey2):
    return lambda x: true_test_monkey2 if test_num2 in x else false_test_monkey2
...
def read_monkeys(file_lines):
    ...
    for file_line in file_lines:
        ...
        if file_line[0:6] == 'Monkey':
            ...
            start_items = [prime_factors(int(item)) for item in file_line.split(':')[1].strip().split(',')]
            if read_debug:
...
calc_debug = True
for mround in range(750):
    print(mround)
    debug_times = False if mround < 740 else True
    if debug_times:
        round_st = time.time()
